...
| FR-RT-17 | S | `live` | Offer a quantised publication of the same model when one would materially cut the download, naming the publisher when it is not the model's owner. Suggest, never substitute: a quantised build is not the same model. |
