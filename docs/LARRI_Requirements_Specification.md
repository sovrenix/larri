...
|---|---|---|---|
| FR-CRIT-01 | M | `live` | Accept hardware criteria: GPU model (e.g. `A100`, `4090`, `H100`), GPU count **and a ceiling on it**, minimum VRAM per GPU and in aggregate, CPU cores, RAM, disk. The count bounds and the aggregate floor are pushed to the provider's own query where it has one and re-applied in ranking where it does not, because a bound that holds only where it happens to be implemented is not a bound. |
...
| FR-SRCH-03 | M | `live` | **Select the cheapest offer that meets the criteria and passes the safety floors.** Criteria are a hard filter; fit is a filter, not a scoring term — once the model is known to run, VRAM headroom has no further business competing with price. Every exclusion records a typed reason and evidence, and the selection prints what it rejected alongside what it chose: an operator asking "why not the cheap one" gets the reason, not a score. The reason names the question that failed: fit is `engine-unsupported` (a vendor, compute capability or driver the engine's image cannot run on), `network-below-floor`, or `insufficient-vram`. Every fit refusal was once filed as insufficient VRAM — a survey for a 1.5B model reported 410 offers short of memory, 409 of them cards the vLLM image cannot run on. |
...
| FR-SRCH-13 | M | `live` | Rank on the **cost of a working endpoint over a session**, not on the hourly rate. The image and the weights are billed at that rate while they download, so the cheapest listing is regularly the dearest rig; the session length that balances the two is a setting (`--session`), not an assumption. |
| FR-SRCH-14 | M | `live` | Exclude hosts whose provider **withdrew** verification, and offer the tier as a filter. Across a 400-offer sample the reliability scores of verified, unverified and deverified hosts were 0.9960, 0.9930 and 0.9929 — indistinguishable — while price split cleanly, so the cheap end that ranking selects is disproportionately the demoted tier. |
| FR-SRCH-15 | S | `live` | Floor the host's download link (`--min-netspeed`) and state the expected time and cost to reach ready **before** the confirmation, warning when the download dominates the bill. |
| FR-SRCH-16 | M | `live` | Consider **multi-GPU hosts** on the same terms as single-GPU ones, sizing against the aggregate VRAM of the cards the chosen runtime can actually place the model on rather than the number in the listing. A model past the largest rentable single card is the ordinary reason to want such a host, so this is the default rather than an opt-in; `--gpus` and `--max-gpus` narrow it. Reported unsatisfiable is reserved for a market that genuinely cannot serve the model, and the report says how many offers were weighed and how much VRAM the best of them could have reached. When the operator's own `--max-gpus` ceiling ruled the fit out, the report names the multi-GPU host that would have fitted. A multi-GPU placement is explained per card, and an engine that splits by layer is said to add memory rather than speed. |
...
| FR-PROV-03 | M | `live` | Treat create as possibly-succeeded on timeout or transport error: reconcile against the provider before retrying, never blind-retry. |
...
| FR-PROV-05 | S | `live` | On provisioning failure attributable to the host (not the model or config), fall back to the next-ranked offer, up to a bounded number of attempts, and report every attempt and its cost. **Falling back means excluding what failed**, and what that is depends on the provider: a marketplace names the machine, so the machine goes; a provider that places the pod itself names none, so the listing goes. Excluding nothing is not a fallback — it re-ranks an unchanged market, picks the same offer, and buys the same failure until the attempts run out, which is what a probe against a provider refusing every create showed. A second failure on the same GPU model retires the model: precise first, wider on evidence. |
| FR-PROV-07 | M | `live` | Verify the local port is free before spending. Once the host is reachable, verify its compute-capability and CUDA floors, sum VRAM across every card, and check access to the weight source before downloading weights. |
| FR-PROV-08 | S | `live` | Once the host is reachable, distinguish a host with no route anywhere from one that merely cannot reach the weight source, by probing a control host alongside it. The first is a bad rental; the second is a good rental in a region that cannot route there, and its remedy is a mirror (`--hf-endpoint`) rather than a different machine. |
| FR-PROV-09 | M | `live` | Size the rented disk to the weights **before the search**, since the search filters on it. A disk nobody named becomes the weights plus the runtime image with headroom, never below the 60 GB every rental used to get; a disk the operator named that cannot hold them is refused with the figure that would, never quietly raised, because it is a setting that spends. The disk must also be where the weights are written: on RunPod the operator's figure sizes the volume at `/workspace` while the container disk is a fixed 20 GB, so LARRI's data directory and the Hugging Face cache are linked onto the volume at boot — by steps that cannot fail, since they run before sshd and a script that stops there strands a billing pod. What the link does not achieve, a free-space check before the download catches, in seconds rather than twenty gigabytes in. Where both a VRAM and a disk shortfall apply, the VRAM one is reported — the disk is one flag, and naming it first sends the operator to fix it only to meet the larger problem on the next run. |
| FR-PROV-10 | M | `live` | **Quote what the rental bills, and account for it at the billed rate.** Ask the search for the disk being rented: Vast prices eight gigabytes of storage into `dph_total` when not told otherwise, so an 86 GB rental was ranked at $0.351/hr and billed $0.394/hr, and a host with cheap GPUs and dear storage ranked above one that was cheaper overall. Read provider units as published — Vast's `storage_cost` is dollars per gigabyte per **month**, and read as dollars an hour it added $0.20/hr to every rig. Journal the rate the provider reports once an instance exists, not the quote, with storage carried as the separate rate the rental bills while `STOPPED` rather than added on top of it — RunPod charges a stopped volume twice what a running one costs, so it is not a part of the running rate. RunPod bills disk beside a GPU rate that includes none — a live A40 pod on 220 GB reported `costPerHr` 0.49, the catalogue rate exactly — so its quote and its instance rate add the container disk and volume, and a stopped pod is costed at its volume. Journal entries record which rules they were written under; entries from before carry the quote and Vast's per-month storage figure, and are read by those rules, at the billed rate the rig's snapshot kept when there is one. |
| FR-PROV-11 | M | `live` | **Offer only what the create call accepts.** The catalogue and the create call are different APIs and disagree: RunPod's catalogue lists `NVIDIA RTX PRO 4500 Blackwell Server Edition` priced and in stock where `POST /pods` accepts `NVIDIA RTX PRO 4500 Blackwell`, plus MIG slices and a literal `unknown` that no create will place — each one a failed attempt when it ranks cheapest. Read the accepted types from the schema the create endpoint publishes and validates against, rather than from a list in the source; when it cannot be read, drop only what is known not to be rentable and say the check was weaker, since a search that offers nothing costs the run. |
| FR-PROV-12 | M | `live` | **Offer low stock only when the operator allows it, and say so either way** (`--allow-low-stock` on `up`, `offers` and `tui`; `allow_low_stock` in profiles, the profile editor and the MCP tools; issue #3). Low stock is not unplaceable — five of six Low-stock RunPod Secure Cloud pods probed on 2026-09-14 were placed at once, and the sixth was refused with nothing created — but a create against it can be refused, so spending attempts on it is the operator's choice. Allowed, a low-stock offer is ranked like any other and marked wherever it is shown before money is spent. Not allowed, the skipped types are named as low stock, and a request that nothing in stock can satisfy names the low-stock offer that would, and the flag that considers it — including when the provider refuses the criteria outright, which is where the issue met it: `--gpu B200` answered "nothing rentable" while B200s were listed. A provider with no notion of stock is not searched a second time. |
...
| FR-RT-02 | M | `live` | Select the runtime automatically from the `ModelSpec` when unspecified (e.g. GGUF → llama.cpp; safetensors + large VRAM → vLLM), and allow explicit override. Every surface selects from the model it was asked for: the MCP server chose from no model at all, so a named `.gguf` file or a Q4_K_M request was planned on vLLM. |
...
| FR-RT-04 | M | `live` | Launch the runtime with an OpenAI-compatible HTTP surface and a **stable served-model name** independent of the upstream repo path. A ref naming a GGUF file is served under the file's model name — `qwen3-8b-q8_0`, not `qwen3-8b-q8_0.gguf`, and never the name of its first part. |
...
| FR-RT-06 | M | `part` | Stream bootstrap progress (image pull, weight download percentage, server start) to the operator; a multi-GB download must not look like a hang. vLLM reports it from the weights cache during the readiness wait, and llama.cpp from the weights directory while its own download runs — the phases differ because the engines fetch at different points. Ollama reports phases but no percentage, which is the remaining gap. The measurement follows links: on RunPod the Hugging Face cache is linked onto the volume, and `du` given a link measures the link, so a live download read "2 MB of 2.9 GB (0%)" from start to finish. It counts bytes written, not blocks allocated: a filesystem reserving space ahead of a growing file read 96% while the first of two parts was arriving, then fell back to 87%. A progress measurement is bounded and cancelled when the download ends, so a host that stops answering one cannot hold a bring-up. |
...
| FR-RT-16 | S | `live` | Derive the runtime's hardware floors **from the pinned image** rather than from the engine's documented support matrix, and refresh them together when the pin moves. The two disagree: `vllm/vllm-openai` ships no Volta kernels while vLLM's matrix still lists 7.0, and that gap rented three V100 boxes that could never have loaded one. The image also fixes the GPU vendor: every image LARRI runs is CUDA, so an AMD card is not a fit for it at any price — the MI300X, 192 GB at $2.39/hr, is the cheapest large card RunPod lists, and an unreported compute capability let it through. A provider that does not say who made a card still passes. |
| FR-RT-17 | S | `live` | Offer a quantised publication of the same model when one would materially cut the download, naming the publisher when it is not the model's owner. Suggest, never substitute: a quantised build is not the same model. Within a repository, a quantisation is one download however many times it is published: unsloth ships Llama-3.3-70B's Q6_K in two folders, and adding both priced it at 115.8 GB, so the advice offered the larger Q8_0 as 35% less to fetch. |
| FR-RT-18 | S | `live` | Name the tool-call parser from the model family so agent clients work without configuration, and refuse to guess for an unrecognised family — a mismatched parser emits malformed calls, which is worse than an honest refusal. |
| FR-RT-11 | M | `part` | Use project-maintained, pre-baked runtime images pinned by content digest rather than composing stock images at boot. Select the image variant against the host's reported driver/CUDA version as a **search filter**, so an incompatible host is excluded before it is rented, not discovered after. Fall back to a stock image with a warning when no compatible variant exists. **Now a prerequisite rather than an optimisation:** RunPod supplies no SSH of its own, so a pod is reachable only if its image runs `sshd` — and upstream engine images do not (§5.5). |
| FR-RT-12 | M | `live` | Resolve model facts by live fetch of the model's `config.json` from Hugging Face, cached by resolved commit; operator override wins, and unresolvable facts are a hard error before spend, never a guess. *A fact nothing consults is not unresolvable:* the parameter count feeds only the weights estimate, so a repository publishing exact file sizes is sized without it — requiring it turned away models that could be sized precisely. The base model is still asked when it is missing, and its absence refuses only a plan that would have to estimate. Gated repositories exercise the Hugging Face token during sizing, so a bad token fails before anything is billed. |
...
| FR-RT-21 | M | `live` | Size the weights from the **measured file size** the repository publishes, and fall back to the parameter-count estimate only when it does not. The estimate is two approximations multiplied — a rounded parameter count and a table of average bits per weight no publisher is obliged to agree with — and it fails in the direction that OOMs: unsloth's `UD-IQ1_M` is 3.31 bits per weight where its name says 1.75, so a 180B model sized from the table came out at 39 GB against a real 69 GB and would have selected a single card for a four-card model. A measurement also means a quantisation naming scheme the table has never seen still sizes. The figure belongs to one file, so it is never carried across to a suggested alternative quantisation. |
| FR-RT-22 | M | `live` | **Settle the weight format before choosing a file, in the daemon.** An engine's default quantisation applies before a repository is listed, not after: `larri up` on a GGUF repository resolved the file with no quantisation and took full precision — 15.3 GB where the documented Q4_K_M default is 4.7 — and surfaces that resolved against no model at all could not use llama.cpp. A ref naming a file (`owner/repo/path/file.gguf`) is checked against the listing: sized by its repository, measured like any other, refused when it is missing, a draft head, or a quantisation other than one named alongside it, and recorded under the quantisation the file carries. A named file whose name carries no quantisation needs one named alongside it, rather than taking the engine default on no evidence, and part names keep the extension as listed, whatever its case. |
| FR-RT-19 | M | `live` | Derive the shard degree from **how the engine splits a model**, not from the card count, and use the same rule in selection and at launch. A layer split takes any number of cards; tensor parallelism takes only a degree that divides the model's attention heads, and vLLM rejects one that does not at engine init — on a machine already billing. A host whose cards the engine cannot all reach is sized on the ones it can, and the shortfall says so, since 144GB advertised and 67GB short of 158GB reconcile only once the degree is named. An unreported head count falls back to the largest power of two rather than emptying the market or licensing a degree the engine may refuse. Verified at the margin: Llama-3.3-70B Q6_K planned at 22.4 of 22.8 GB usable per card loaded across 3× RTX 3090 as a layer split and served, at 14 tokens/s. A MIG slice is part of a card, and a CUDA process sees one MIG device however many a pod holds, so a MIG type is offered as one slice: RunPod prices a PRO 6000 MIG 1g.24gb at up to 22 slices, which as a split model would read as 528 GB the engine could load into 24 GB of. |
| FR-RT-20 | M | `live` | Acquire **every part** of a multi-part weight file, and write it where the launch will look. llama.cpp finds the remaining shards itself once it holds the first, but it cannot find what was never fetched — and every model large enough to need more than one card is published in parts, so for that engine this is the case rather than an edge case. A quantisation published in a repository subdirectory lands beside the others, because the download created the model directory and never the one inside it. A set the repository does not hold completely is refused before renting — a named part that is missing, or a quantisation still uploading — because the missing part fails the download on the host, reads as the host's fault, and rents the next one to fail the same way. Files are listed at the revision that will be downloaded. |
...
| FR-WIRE-06 | M | `done` | Never write the ephemeral provider host or port into client configuration. |
...
| FR-WIRE-14 | M | `done` | Verify wiring by probing after writing, in every tier. Configuration written correctly but not yet loaded — an application that reads its config only at startup — must be reported as needing a restart, never as success. |
...
| FR-SUP-13 | M | `live` | On restart, rebuild the data plane for a rig still running at the provider: install a **newly minted** SSH key on the running instance through the provider API, reconnect, re-attach to the running server, and republish the endpoint **on the same local port** so wired clients recover without reconfiguration. The lost key must be superseded, never retrieved from storage (FR-STATE-05). Reconnection uses the engine the rig was brought up with — every rig was adopted as vLLM, so a llama.cpp or Ollama rig could not be reconnected — and the provider's view of the instance replaces the snapshot's before anything is journalled. An instance found gone ends the rig with the reason recorded. A reconnected rig accepts the stored client keys, so clients configured against the rig keep working; it used to mint a key and never print it, leaving a rig no client could reach. A reconnected rig is supervised like one brought up in the same process — idle reclamation and the budget apply — where it used to be held with no supervisor at all. |
...
| FR-SUP-16 | M | `live` | Report a rig that cannot be reconnected as **destroyable but not reconnectable**, naming its ongoing hourly cost. A failed resume must never read as a command that changed nothing. |
...
| FR-SUP-19 | M | `live` | **A rig has at most one holder** — the process serving its endpoint and supervising it — enforced by a kernel lock that a crashed or killed holder releases by dying. `larri resume` refuses a rig another process holds, naming its pid, where it used to start a second tunnel and supervisor. `larri status` and `larri_status` say who holds a billing rig, and a billing rig nobody holds is shown with no endpoint and no idle reclamation, which is the state a rig is in after its holder dies. Exercised live on Vast.ai (2026-09-14): `status` named the holder from PROVISIONED onwards; `resume` and `resume -d` were refused by pid while a rig was held; after `kill -9` of the holder, `status` reported no endpoint and no idle reclamation and `resume -d` took the rig; `larri down` from another shell ended rigs whose holders then exited within ten seconds. |
...
| ID | Pri | Status | Requirement |
|---|---|---|---|
| FR-DEL-01 | M | `part` | A named rig is taken at the operator's word — `larri down <id>` acts on that rig whatever state it is in, and only the unnamed form looks for something billing, since the two disagreeing once left a rig no command could clear. `larri down` destroys the rig, reverts client wiring, closes the tunnel, and reports total cost. |
| FR-DEL-02 | M | `live` | Teardown is idempotent — safe to invoke repeatedly and from any state including `FAILED` and `ORPHANED`. A destroyed rig stays destroyed: the store refuses to move one back to a live state, and cost replay ignores anything journalled after its end. Two processes holding one rig is the ordinary case — one serving it, one tearing it down — and the stale one wrote DEGRADED over DESTROYED three probes later, which billed a pod that no longer existed and would have refused the next `larri up` as a second rig. Exercised live: a rig ended by `larri down` in a second terminal ended the supervising `larri up` within a poll, which adopted the record and journalled no second ending. A second teardown working from a stale copy adopts the first one's record rather than replacing its reason, whether it finds the rig ended when it starts or only when it writes; a rig file that cannot be read blocks every move except teardown. Every snapshot write passes the same guard as a transition, and the check and the write share a per-rig lock across processes: serving and adoption save the rig directly, and a bring-up that saved its endpoint after `larri down` had finished wrote a live state back over DESTROYED. A bring-up whose save is refused this way stops. |
...
| FR-DEL-06 | M | `live` | Provide `larri orphans` to list and destroy unaccounted-for resources across all configured providers. Without `--provider`, every provider that opens is swept, and each orphan is destroyed through the provider that listed it; one that cannot be listed is reported as not checked. Sweeping only the default hid a RunPod pod on a machine configured for Vast, and the MCP destroy tool, given only an id, sent it to the wrong provider — whose "not found" reads as confirmed absence. Destroying an orphan now first confirms the instance is at that provider. |
...
| FR-DEL-12 | M | `done` | Close out a create that returned no instance: journal **and** snapshot the failure together, then ask the provider whether anything carrying the rig's label exists. One that does is torn down rather than left for a later sweep; one that demonstrably does not ends the rig on the spot, so the accrual stops at the failure. A create whose outcome is *unknown* concludes nothing from a single listing — the instance can still appear — and stays FAILED and loud. Writing only the journal left a rig reading SELECTED in `larri status` while the journal billed it at $1.39/hr for two days, with no command able to clear it. `larri down` of a rig with no instance on record follows the same rule: when the provider cannot be asked, the rig stays billable and the command asks for a retry, since recording it destroyed stopped the cost of whatever a lost create had made. |
| FR-DEL-13 | S | `done` | Let the operator correct a record LARRI cannot settle: a destroyed rig that never had an instance on record, left accruing by a teardown made before failed creates settled themselves. Only the operator can establish that nothing was created — a listing today shows what exists now, not what existed then — so the correction records their account of how they checked, as theirs, and is refused where the record or the provider proves otherwise: an instance id on record, or an instance carrying the rig's label now. Not an agent tool, since it records a verification made outside LARRI. |
| FR-DEL-14 | M | `live` | Act on an existing rig only through **the provider that holds it**. Another provider answers "not found" for an instance it never held, which is exactly what confirmed absence looks like: `larri down`, `larri resume` and the MCP tools opened the configured default, so a RunPod rig torn down on a machine configured for Vast could be recorded destroyed while it went on billing. The daemon refuses the mismatch, so no surface can make it. |
| FR-DEL-08 | M | `live` | Record a **typed termination reason** for every rig that stops existing, resolved at the moment of the decision and journalled with the teardown intent — never reconstructed afterwards. It must carry the deciding actor (operator, configured policy, provider, or LARRI fault), the reason code, and the evidence behind it. A reason without evidence is not an explanation. The entry that ends the rig carries it: the journal declared the field and nothing wrote it, so the reason lived only in the snapshot, and cost — replayed from the journal — never saw evidence recorded at teardown. |
...
| FR-UI-14 | M | `live` | **Detach.** `larri up -d` and `larri resume -d` return once the rig is ready — or with the error if it is not — leaving a larri process of its own holding it, so an agent driving a shell gets a command that finishes. The holder is the same orchestrator and supervisor as a foreground `up`: idle reclamation and the budget apply, and `larri down` ends it. Confirmation stays with the launching command: from a terminal it runs the ordinary survey and prompt and hands the holder the agreed price as its ceiling, so a market that moved in between cannot rent something dearer; without a terminal `--detach` needs `--yes` rather than implying it. The holder writes to a `0600` log that never carries a key value — the key reaches only the launching command, over an inherited pipe — and `--json` prints the outcome as one object on stdout, with every other line on stderr. The launcher checks the local port before it asks, and a holder's fallback stays at or below the agreed price — the launcher says so when that leaves nothing to fall back on. Stopping a holder that is serving tears the rig down, as Ctrl-C does to a foreground `up` — whether `up -d` or `resume -d` started it, so a shutdown never leaves a resumed rig billing with nothing watching it; a Ctrl-C in the launcher before READY abandons a reconnect and leaves the rig running, as a foreground `resume` does. With `--json`, a refused flag and a declined offer are reported as objects too. Exercised live on Vast.ai (2026-09-14): `up -d` from a terminal (the agreed $0.102/hr passed down as `--max-price 0.1023`), `up -d --yes --json` and `resume -d --json` without one, each returning at READY with the holder supervising; SIGTERM to a launcher mid-bring-up tore the rig down; a holder whose agreed host failed had nothing at that price to fall back on. |
| FR-UI-15 | S | `live` | **Logs.** `larri logs [-f] [-n N] [rig]` prints what LARRI wrote while holding a rig — bring-up, supervision, teardown — from the log of every detached process that has held it, oldest first, as `docker logs` prints a container's: the rig by id or unique prefix (the newest rig with a log when none is named), `--tail` from the end, `--follow` until nobody holds the rig and its last holder has exited, moving to a new holder's log when the rig changes hands. A rig held only in the foreground has no log and the command says where its output went. `larri_logs` returns the same log for a rig the MCP server is not serving, and the engine's own log for the one it is. The hold is kept through a teardown, so a follower sees how the rig ended and nothing reconnects to a rig being destroyed. Exercised live on Vast.ai (2026-09-14): `larri logs -f` on a lowercase id prefix followed a llama.cpp bring-up from PROVISIONED to READY and exited a second after `kill -9` of the holder; after `resume -d`, `larri logs` showed both holders' logs oldest first, and `logs -f -n 0` printed only new lines — the idle warning, the policy teardown and DESTROYED — and exited as the holder did. |
...
| FR-SEC-15 | M | `live` | Request a provider port mapping for SSH only. A container port that was never mapped is unreachable regardless of what listens on it, which makes this the primary network control — provider-enforced, needing no privilege inside the instance and immune to a mistake in a bootstrap script. |
...
| FR-SEC-19 | M | `live` | Never declare a port that the provider would publish. On RunPod a declared HTTP port is automatically served through a public proxy protected only by Pod-ID obscurity; declaring the runtime port there would publish an inference endpoint to the internet. |
...
| FR-SEC-32 | M | `live` | **Store client keys, not their values.** `larri token create <name>` makes a 256-bit key for one client and shows it once; only its SHA-256 is kept, in a `0600` file in the state directory, so a lost key is revoked and replaced rather than recovered. Every bring-up — `up`, `resume`, the TUI, the MCP server — accepts every stored key, and re-reads the file when it changes, so `larri token revoke` takes effect on a rig that is already serving. The first `larri up` with none stored creates one called `default` once the rig is ready, so a run that never serves leaves no key nobody saw. A key file that cannot be read is refused before renting, since it would match no key; a rig whose stored keys break after that — or whose `default` key cannot be written — is given a key of its own rather than served with none. `--new-key` adds a key for one rig only; the MCP server gives agents one by default, returned by `larri_status`, since an agent's key should go with its rig. Exercised live on Vast.ai (2026-09-14) through `up` and `resume`: a key created while a rig served was accepted on its next request and `default`, revoked, was refused with 401 on the next, with no restart; no key value appeared in the state directory, the config, or any log. The TUI and MCP paths are unit-tested only. |
...
| ID | Pri | Status | Requirement |
|---|---|---|---|
| FR-WHISPER-01 | M | `done` | Run a speech-to-text server on the rented host exposing OpenAI's `/v1/audio` surface, declared as its own protocol. Compatibility with the audio clients is not the chat contract, and a caller that needs a completion must be told so before renting rather than by a 404 on a rig that is already billing. |
| FR-WHISPER-02 | M | `done` | Measure the model repository **before the create call** and refuse a name that does not resolve or a gated repository the operator's token cannot read. One listing fetch, locally, costs nothing; discovering either on a rented host costs the rental (§4a). |
| FR-WHISPER-03 | M | `done` | Size the card from the **resident** weights and the disk from the **downloaded** ones. CTranslate2 quantises at load rather than at publication, so asking for int8 does not shrink the fetch and does shrink the card — and the saving is the weight difference rather than half, because the working set is computed in float16 either way. Conflating the two mis-sizes one of them in a direction that only shows up on the rental. |
| FR-WHISPER-04 | M | `done` | Discover how the image exposes its server rather than assuming a module path, a console script or an environment variable name. Those are facts about somebody else's container; carrying the evidence into the failure is what makes the error actionable once the machine is gone. |
| FR-WHISPER-05 | M | `done` | Declare readiness only after a transcription has round-tripped through the tunnel, and only after confirming the server loaded the model that was asked for. A setting this adapter got wrong does not fail — the server starts with its own default and transcribes happily with a model the operator did not choose. |
| FR-WHISPER-06 | M | `done` | Generate the readiness audio rather than shipping a recording, so no clip lives in the repository and no licence attaches to one. It must be audible rather than silent: a voice-activity filter can skip a silent file without running the model at all, which would make readiness prove the HTTP path and nothing else. |
...
| FR-WHISPER-08 | S | `done` | Name each wired client separately, so each holds its own credential and each can be attributed and revoked on its own. The name is the operator's word for their application; nothing is detected, because nothing is written. |
...
- **AC-1.3** `larri status` shows state, provider, hardware with its card count, instance,
  hourly price, elapsed time, accrued cost, and the model and runtime being served — and shows
  the provider's billed rate beside the quoted one when they differ, since that difference is
  money the ranking never saw. The MCP status tool renders the same summary.
