...
| FR-PROV-10 | M | `live` | **Quote what the rental bills, and account for it at the billed rate.** Ask the search for the disk being rented: Vast prices eight gigabytes of storage into `dph_total` when not told otherwise, so an 86 GB rental was ranked at $0.351/hr and billed $0.394/hr, and a host with cheap GPUs and dear storage ranked above one that was cheaper overall. Read provider units as published — Vast's `storage_cost` is dollars per gigabyte per **month**, and read as dollars an hour it added $0.20/hr to every rig. Journal the rate the provider reports once an instance exists, not the quote, with storage carried as the part of that rate still charged while `STOPPED` rather than added on top of it. RunPod bills disk beside a GPU rate that includes none — a live A40 pod on 220 GB reported `costPerHr` 0.49, the catalogue rate exactly — so its quote and its instance rate add the container disk and volume, and a stopped pod is costed at its volume. Journal entries record which rules they were written under; entries from before carry the quote and Vast's per-month storage figure, and are read by those rules, at the billed rate the rig's snapshot kept when there is one. |
