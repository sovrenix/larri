...
| FR-SUP-18 | M | **Never place a provider credential on a rented host.** The host may only stop the runtime and halt itself; that ends compute billing and leaves storage, and every report of it must say so rather than imply a teardown. Real teardown stays a provider call from the operator's machine (FR-SEC-18). |
