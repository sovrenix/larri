...
| FR-SRCH-16 | M | `live` | Consider **multi-GPU hosts** on the same terms as single-GPU ones, sizing against the aggregate VRAM of the cards the chosen runtime can actually place the model on rather than the number in the listing. A model past the largest rentable single card is the ordinary reason to want such a host, so this is the default rather than an opt-in; `--gpus` and `--max-gpus` narrow it. Reported unsatisfiable is reserved for a market that genuinely cannot serve the model, and the report says how many offers were weighed and how much VRAM the best of them could have reached. |
