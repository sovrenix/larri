...
func TestPickFollowsTheHeuristic(t *testing.T) {
	...
	cases := []struct {
		...
		{"an 8-bit gguf quantisation is llama.cpp",
			core.ModelSpec{Ref: "org/model", Quantization: "q8_0"}, fits, 1, core.RuntimeLlamaCpp},
		{"an i-quant is llama.cpp",
			core.ModelSpec{Ref: "org/model", Quantization: "iq3_xxs"}, fits, 1, core.RuntimeLlamaCpp},
		{"awq is vllm",
			core.ModelSpec{Ref: "org/model-AWQ", Quantization: "awq"}, fits, 1, core.RuntimeVLLM},
		{"gptq is vllm",
			core.ModelSpec{Ref: "org/model-GPTQ", Quantization: "gptq-int4"}, fits, 1, core.RuntimeVLLM},
