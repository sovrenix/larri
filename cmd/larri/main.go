...
const usage = `larri — Local Agent for Remote Rigging of Inference
  ...
  larri comfy   rent a GPU for a ComfyUI workflow and open it locally
...
func cmdDown(ctx context.Context, args []string) error {
	...
	// A rig that renders images holds the only copy of them, and this command
	// cannot fetch them: the rig's SSH identity is ephemeral and deliberately
	// never persisted (FR-STATE-05), so a second process has no way to reach
	// the host. Destroying is therefore irreversible here in a way it is not
	// for an inference rig, where nothing of the operator's lives on the
	// machine.
	//
	...
	discard := fs.Bool("discard-outputs", false,
		"destroy a rig whose rendered outputs have not been collected")
	_ = fs.Parse(args)
	...
	if target.Runtime == core.RuntimeComfyUI && !*discard {
		return fmt.Errorf(
			"down: rig %s holds renders that exist only on the host: "+
				"stop the comfy session to collect them, or --discard-outputs",
			target.ID)
	}
